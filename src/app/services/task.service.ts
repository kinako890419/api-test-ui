...
export class TaskService {
  ...
  getTaskById(projectId: number, taskId: number): Observable<TaskDetailResp> {
    return this.http.get<TaskDetailResp>(`${environment.apiBaseUrl}/projects/${projectId}/tasks/${taskId}`);
  }

  addTask(projectId: number, body: AddNewTaskReq): Observable<ResponseMsg> {
    return this.http.post<ResponseMsg>(`${environment.apiBaseUrl}/projects/${projectId}/tasks`, body);
  }

  updateTask(projectId: number, taskId: number, body: EditTaskReq): Observable<ResponseMsg> {
    return this.http.patch<ResponseMsg>(`${environment.apiBaseUrl}/projects/${projectId}/tasks/${taskId}`, body);
  }

  deleteTask(projectId: number, taskId: number): Observable<ResponseMsg> {
    return this.http.delete<ResponseMsg>(`${environment.apiBaseUrl}/projects/${projectId}/tasks/${taskId}`);
  }

  downloadAttachment(projectId: number, taskId: number, attachmentId: number): Observable<Blob> {
  ...
  removeTagFromTask(projectId: number, taskId: number, tagId: number): Observable<ResponseMsg> {
